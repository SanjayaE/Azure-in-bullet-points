...
- **Resource groups**
  ...
  ![img](https://i.imgur.com/1VhF4m1.png)
...
- **Microsoft Azure Policies**
  ...
  - **Custom Policies**
    ...
    - E.g.
      ...
      ```
  - **Policy Defination**
    - condition that the policy will report on (e.g location)
    - Enforce if configured to do so (e.g not allowing to create VM on some locations)
  - **Policy Assignment**
    - Definition that has been assigned to a scope (what do you applying the policy to)
    - Assignments are inheritaed by all child resources
  - **Policy parameters**
    - Passed to policy
    - Enable policy reuse (reduce the number of definitions)
  ...
  - **Policy Initiative Definations**
   ...
   - Same as the policy there are initiative parameters and initiative assignment
...
- **Log types**:
  ...
  - **Diagnostic Logs**
    ...
    - Resource logs -activity for resoures within an azure subs  (e.g Storage account)
    - Tenant logs - activity on resoures outside an azure subs (e.g azure AD)
  - **Guest OS Diagnostic Logs**
    - Logs collected from an agent on the resources such as VM
    - Compute resources: Guest OS (e.g. syslog for Linux, event logs for Windows)
    - Gather data from the OS
    - Gather data from application
  ...
  - **Activity Logs**
    ...
    - Use the Activity Log, to determine the what, who, and when for any write operations (PUT, POST, DELETE) taken on the resources in your subscription.
    - E.g.
      - *Who created VM?*
